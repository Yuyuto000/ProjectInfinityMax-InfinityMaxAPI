...
public class NeoForgeEnergyCompat implements IEnergyCompat {
    ...
    public boolean isEnergyItem(Object obj) {
        ...
        return s.getCapability(CapabilityEnergy.ENERGY).isPresent();
    }

    @Override
    public int getEnergy(Object obj) {
        ItemStack s = (ItemStack)obj;
        return s.getCapability(CapabilityEnergy.ENERGY).map(IEnergyStorage::getEnergyStored).orElse(0);
    }

    @Override
    public int getMaxEnergy(Object obj) {
        ItemStack s = (ItemStack)obj;
        return s.getCapability(CapabilityEnergy.ENERGY).map(IEnergyStorage::getMaxEnergyStored).orElse(0);
    }

    @Override
    public int receiveEnergy(Object obj, int amount, boolean simulate) {
        ItemStack s = (ItemStack)obj;
        return s.getCapability(CapabilityEnergy.ENERGY).map(cap -> cap.receiveEnergy(amount, simulate)).orElse(0);
    }

    @Override
    public int extractEnergy(Object obj, int amount, boolean simulate) {
        ItemStack s = (ItemStack)obj;
        return s.getCapability(CapabilityEnergy.ENERGY).map(cap -> cap.extractEnergy(amount, simulate)).orElse(0);
    }
}
